/* =========================================================================
   SUPER ABEL123 BROS MAKER
   A platformer engine with 40 built-in levels + a level editor.
   ========================================================================= */
...
const WORLD_THEMES = [
  { id: 1, name: 'Grassland', theme: 'grass' },
  { id: 2, name: 'Desert', theme: 'desert' },
  { id: 3, name: 'Ice Plains', theme: 'ice' },
  { id: 4, name: 'Caverns', theme: 'castle' },
  { id: 5, name: 'Skylands', theme: 'grass' },
  { id: 6, name: 'Lavaland', theme: 'castle' },
  { id: 7, name: 'Night Forest', theme: 'night' },
  { id: 8, name: 'Final Castle', theme: 'castle' },
];
...
function generateLevel(levelIndex) {
  ...
  return {
    id: levelIndex,
    name: `${themeInfo.name} ${stage + 1}${isBoss ? ' - BOSS' : ''}`,
    world: world + 1,
    stage: stage + 1,
    theme,
    isBoss,
...
class Game {
  ...
  update(dt) {
    ...
    this.time -= dt / 60;
    if (this.time <= 0 && !this.finished) { this.loseGame('time'); return; }
  ...
  loseLife() {
    this.lives--;
    if (this.lives <= 0) { this.loseGame('lives'); }
    else {
      // respawn at start
      const p = this.player;
  ...
  _applyPowerup(kind) {
    ...
    if (kind === POWERUP.MUSHROOM) {
      ...
      this.floatingTexts.push(new FloatingText(p.x, p.y - 10, 'Bigger!', '#e74c3c'));
    } else if (kind === POWERUP.FIRE) {
      p.power = 'fire';
      this.floatingTexts.push(new FloatingText(p.x, p.y - 10, 'Fire!', '#ff8844'));
    } else if (kind === POWERUP.STAR) {
      p.invincible = 600;
      this.floatingTexts.push(new FloatingText(p.x, p.y - 10, 'Invincible!', '#ffd700'));
  ...
  _updateCamera() {
    ...
    this.camera.x = clamp(this.camera.x, 0, Math.max(0, this.level.width * TILE - viewW));
    // Center the level vertically within the available view height so
    // extra vertical space (e.g. wide landscape screens) becomes symmetric
    // sky margin above/below rather than empty space only at the bottom.
    const levelPxHeight = this.level.height * TILE;
    this.camera.y = -Math.max(0, (viewH - levelPxHeight) / 2);
  }
  // Dynamic zoom: always fit the full level height (+ a margin for the HUD/
  // controls) inside the canvas, so landscape (wide+short) screens zoom out
  // instead of cropping the player and ground off-screen.
  _scale() {
    if (!this.level) return 1;
    const cw = this.canvas.width, ch = this.canvas.height;
    const levelPxHeight = this.level.height * TILE;
    // Reserve a bit of vertical space so HUD text / bottom controls don't
    // overlap the playfield too much, especially on short landscape screens.
    const verticalMargin = clamp(ch * 0.12, 30, 90);
    const availableH = ch - verticalMargin;
    let scale = availableH / levelPxHeight;
    // Cap how many tiles are visible horizontally/vertically so very wide
    // or very tall screens don't zoom in so far that the world feels tiny
    // and empty, or zoom out so far the player becomes hard to see.
    const minVisibleTilesWide = 13;
    const maxVisibleTilesWide = 26;
    const maxScaleByWidth = cw / (minVisibleTilesWide * TILE);
    const minScaleByWidth = cw / (maxVisibleTilesWide * TILE);
    scale = clamp(scale, minScaleByWidth, maxScaleByWidth);
    return clamp(scale, 0.45, 1.4);
  }
  ...
  render() {
    ...
    // Sky gradient (drawn in raw canvas space, before scaling)
    ...
    // Parallax background hills/stars (raw canvas space)
    ...
    const endCol = Math.min(this.level.width - 1, Math.ceil((this.camera.x + viewW) / TILE) + 1);
...
/* =========================================================================
   SAVE DATA / PROGRESS
   ========================================================================= */
const SAVE_KEY = 'super_abel123_bros_maker_progress_v1';
const CUSTOM_LEVELS_KEY = 'super_abel123_bros_maker_custom_levels_v1';
...
const App = {
  ...
  openLevelSelect(mode) {
    ...
    document.getElementById('ls-title').textContent = mode === 'campaign' ? 'Adventure — Choose a level' : 'My Levels';
    ...
    if (mode === 'campaign') {
      WORLD_THEMES.forEach((w, i) => {
        ...
        tab.textContent = `World ${w.id}: ${w.name}`;
  ...
  _renderLevelGrid() {
    ...
    if (this.levelSelectMode === 'campaign') {
      ...
      if (customs.length === 0) {
        grid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; color:#aaa; padding:40px;">You haven\'t made any levels yet.<br>Go to the Level Editor to build your first level! 🛠</div>';
      } else {
  ...
  _bindGameScreen() {
    ...
    this._resizeCanvas();
    // Rotation on mobile browsers doesn't always fire a plain 'resize' event
    // reliably (or fires it before the new dimensions are actually applied),
    // so we listen on every signal that could mean the viewport changed and
    // re-measure a couple of times shortly after.
    const scheduleResize = () => {
      this._resizeCanvas();
      setTimeout(() => this._resizeCanvas(), 60);
      setTimeout(() => this._resizeCanvas(), 250);
      setTimeout(() => this._resizeCanvas(), 500);
    };
  ...
  _loop(t) {
    ...
    if (this.screens.game.classList.contains('active')) {
      // Cheap safety net: if the canvas's backing size no longer matches
      // its on-screen container (rotation, split-screen resize, browser
      // chrome show/hide), fix it before rendering this frame instead of
      // waiting for a resize event that may never fire.
      const wrap = document.getElementById('game-canvas-wrap');
      const canvas = document.getElementById('game-canvas');
  ...
  _togglePause() {
    ...
    if (this.game.paused) {
      this._showMessage('⏸ Paused', '', [
        { label: '▶ Resume', action: () => this._hideMessage() },
  ...
  _onLevelWin(result) {
    ...
    const backLabel = this.playSource.type === 'test' ? '🛠 Back to editor' : '🏠 Menu';
    ...
    if (this.playSource.type === 'campaign' && this.playSource.index < 39) {
      buttons.push({ label: 'Next level ▶', action: () => { this._hideMessage(); this.startCampaignLevel(this.playSource.index + 1); } });
    }
    buttons.push({ label: '🔁 Retry', action: () => { this._hideMessage(); this._replay(); } });
    buttons.push({ label: backLabel, action: backAction });

    this._showMessage('🏆 Level complete!', `Coins: ${result.coins}  ${starText}`, buttons);
  },

  _onLevelLose(reason) {
    const msg = reason === 'time' ? 'Time is up!' : reason === 'fell' ? 'You fell!' : 'You have no lives left!';
    const backLabel = this.playSource.type === 'test' ? '🛠 Back to editor' : '🏠 Menu';
    const backAction = this.playSource.type === 'test'
      ? () => { this._hideMessage(); this.showScreen('editor'); }
      : () => { this._hideMessage(); this.showScreen('menu'); };
    this._showMessage('💀 Game Over', msg, [
      { label: '🔁 Retry', action: () => { this._hideMessage(); this._replay(); } },
...
const EDITOR_PALETTE = [
  ...
  { tile: T.FLAGPOLE, icon: '🏁', label: 'Flag', isSpecial: 'flag' },
  { section: 'Blocks' },
  { tile: T.GROUND, icon: '🟫', label: 'Ground' },
  { tile: T.BRICK, icon: '🧱', label: 'Brick' },
  { tile: T.QUESTION, icon: '❓', label: 'Coin block' },
  { tile: T.QUESTION_POWER, icon: '⭐', label: 'Power block' },
  { tile: T.PLATFORM, icon: '➖', label: 'Platform' },
  { tile: T.ICE_BLOCK, icon: '🧊', label: 'Ice' },
  { tile: T.CLOUD_BLOCK, icon: '☁️', label: 'Cloud' },
  { tile: T.PIPE, icon: '🟩', label: 'Pipe' },
  { tile: T.STAIR, icon: '🔼', label: 'Stairs' },
  { section: 'Danger' },
  { tile: T.SPIKE, icon: '🔺', label: 'Spikes' },
  { tile: T.LAVA, icon: '🌋', label: 'Lava' },
  { tile: T.BOUNCE, icon: '🎯', label: 'Trampoline' },
  { section: 'Items' },
  { tile: T.COIN, icon: '🪙', label: 'Coin' },
  { section: 'Enemies' },
  { entity: ENEMY.GOOMBA, icon: '🍄', label: 'Walker' },
  { entity: ENEMY.JUMPER, icon: '🐸', label: 'Jumper' },
  { entity: ENEMY.FLYER, icon: '🦇', label: 'Flyer' },
  { entity: ENEMY.SPIKY, icon: '🦔', label: 'Spiky' },
  { entity: ENEMY.SHOOTER, icon: '🗿', label: 'Shooter' },
  { entity: ENEMY.BOSS, icon: '👹', label: 'BOSS' },
  { section: 'Power-ups' },
  { powerup: POWERUP.MUSHROOM, icon: '🍄', label: 'Mushroom' },
  { powerup: POWERUP.FIRE, icon: '🔥', label: 'Fire' },
  { powerup: POWERUP.STAR, icon: '✨', label: 'Star' },
  { powerup: POWERUP.WINGS, icon: '🪽', label: 'Wings' },
  { powerup: POWERUP.ONEUP, icon: '💚', label: '1-UP' },
  { section: 'Erase' },
  { tile: T.EMPTY, icon: '🚫', label: 'Erase' },
];
...
class LevelEditor {
  constructor(canvas) {
    ...
    this.scrollY = 0;
    this.zoom = 1;
    this.mode = 'draw'; // 'draw' | 'pan' | 'erase'
    this.onScrollChange = null; // callback(scrollX, maxScrollX) to update UI scrollbar
    this._bindInput();
  }
  ...
  setZoom(z) {
    this.zoom = clamp(z, 0.4, 2);
    this._clampScroll();
  }

  _maxScrollX() {
    return Math.max(0, this.width * TILE * this.zoom - this.canvas.width);
  }
  _maxScrollY() {
    return Math.max(0, this.height * TILE * this.zoom - this.canvas.height);
  }
  _clampScroll() {
    this.scrollX = clamp(this.scrollX, 0, this._maxScrollX());
    this.scrollY = clamp(this.scrollY, -60, Math.max(0, this._maxScrollY()));
    this._notifyScroll();
  }
  _notifyScroll() {
    if (this.onScrollChange) this.onScrollChange(this.scrollX, this._maxScrollX());
  }

  _bindInput() {
    const canvas = this.canvas;

    // A single unified pointer-drag tracker drives both drawing and panning
    // so the same finger/mouse gesture works whichever mode is active —
    // no scroll wheel or right-click required.
    let dragging = false;
    let dragMode = null; // 'draw' | 'erase' | 'pan'
    let lastX = 0, lastY = 0;
    let moved = false;
    let lastPaintedGX = null, lastPaintedGY = null;
    ...
    const getGridPos = (clientX, clientY) => {
      ...
      const x = (cx + this.scrollX) / this.zoom;
      const y = (cy + this.scrollY) / this.zoom;
      return { gx: Math.floor(x / TILE), gy: Math.floor(y / TILE) };
    };

    const paint = (gx, gy, erase) => {
      if (gx < 0 || gx >= this.width || gy < 0 || gy >= this.height) return;
      if (gx === lastPaintedGX && gy === lastPaintedGY && erase === this._lastEraseFlag) return;
      lastPaintedGX = gx; lastPaintedGY = gy; this._lastEraseFlag = erase;
    ...
    const startDrag = (clientX, clientY, isRightClick) => {
      dragging = true;
      moved = false;
      lastX = clientX; lastY = clientY;
      lastPaintedGX = null; lastPaintedGY = null;
      if (isRightClick) {
        dragMode = 'erase';
    ...
    const endDrag = () => { dragging = false; dragMode = null; };

    // Mouse (desktop testing)
    canvas.addEventListener('mousedown', (e) => {
      e.preventDefault();
      startDrag(e.clientX, e.clientY, e.button === 2);
    });
    canvas.addEventListener('mousemove', (e) => { moveDrag(e.clientX, e.clientY); });
    window.addEventListener('mouseup', endDrag);
    canvas.addEventListener('contextmenu', (e) => e.preventDefault());

    // Touch (mobile) — single finger draws/erases/pans depending on mode;
    // two fingers always pans+pinch-zooms regardless of mode, so you can
    // always get around the level even while in Build mode.
    ...
    canvas.addEventListener('touchstart', (e) => {
      ...
      if (e.touches.length === 2) {
        ...
        pinchStartScrollX = this.scrollX;
        pinchStartScrollY = this.scrollY;
    ...
    canvas.addEventListener('touchmove', (e) => {
      ...
      if (e.touches.length === 2 && pinchStartDist) {
        ...
        const newZoom = clamp(pinchStartZoom * factor, 0.4, 2);
        // keep the pinch midpoint anchored to the same world position
        const rect = canvas.getBoundingClientRect();
        const scaleX = canvas.width / rect.width;
        const worldX = (pinchStartMidX - rect.left) * scaleX + pinchStartScrollX;
        const worldY = (pinchStartMidY - rect.top) * scaleX + pinchStartScrollY;
        this.zoom = newZoom;
        this.scrollX = worldX * (newZoom / pinchStartZoom) - (pinchStartMidX - rect.left) * scaleX;
        this.scrollY = worldY * (newZoom / pinchStartZoom) - (pinchStartMidY - rect.top) * scaleX;
        this._clampScroll();
        App._updateEditorZoomLabel && App._updateEditorZoomLabel();
      } else if (e.touches.length === 1) {
        const t = e.touches[0];
    ...
    canvas.addEventListener('touchend', (e) => {
      if (e.touches.length === 0) { endDrag(); pinchStartDist = null; }
      else if (e.touches.length === 1) { pinchStartDist = null; }
    });

    // Desktop convenience: wheel still pans/zooms for anyone testing with a
    // mouse, but is entirely optional — never required on mobile.
  ...
  render() {
    ...
    const startCol = Math.max(0, Math.floor(this.scrollX / this.zoom / TILE));
    const endCol = Math.min(this.width - 1, Math.ceil((this.scrollX + cw) / this.zoom / TILE));
...
Object.assign(App, {
  openEditor(existingData) {
    if (!this.editor) {
      ...
      const scheduleEditorResize = () => {
        this._resizeEditorCanvas();
        setTimeout(() => this._resizeEditorCanvas(), 60);
        setTimeout(() => this._resizeEditorCanvas(), 250);
        setTimeout(() => this._resizeEditorCanvas(), 500);
      };
      window.addEventListener('resize', scheduleEditorResize);
      window.addEventListener('orientationchange', scheduleEditorResize);
      if (window.visualViewport) window.visualViewport.addEventListener('resize', scheduleEditorResize);
      this._renderPalette();
      this._bindEditorModeBar();
      this.editor.onScrollChange = (scrollX, maxScrollX) => this._updateEditorScrollbar(scrollX, maxScrollX);
    ...
    if (existingData) {
      ...
      ed.scrollX = 0; ed.scrollY = 0; ed.zoom = 1;
      ...
      this.editor.clear();
      this.editor.scrollX = 0; this.editor.scrollY = 0; this.editor.zoom = 1;
    }
    this._setEditorMode('draw');
    this._updateEditorZoomLabel();
    this.editor._clampScroll();
  ...
  _resizeEditorCanvas() {
    ...
    if (canvas.width !== w || canvas.height !== h) {
      canvas.width = w;
      canvas.height = h;
      if (this.editor) this.editor._clampScroll();
    }
  },
  ...
  _renderPalette() {
    ...
    EDITOR_PALETTE.forEach(item => {
      ...
      const choose = () => {
        ...
        // Picking a tile/enemy/powerup implicitly means "I want to build" —
        // if the user was in Erase or Move-view mode, bring them back to
        // Build mode so tapping the canvas places what they just picked.
        if (this.editor.mode !== 'draw') this._setEditorMode('draw');
      };
  ...
  _bindEditorModeBar() {
    ...
    modeButtons.forEach(btn => {
      btn.addEventListener('click', () => this._setEditorMode(btn.dataset.mode));
    });
    document.getElementById('ed-zoom-in').addEventListener('click', () => {
      this.editor.setZoom(this.editor.zoom + 0.2);
      this._updateEditorZoomLabel();
    });
    ...
    // Touch-drag the scrollbar track itself as an alternative way to jump
    // to a distant part of the level quickly.
    const track = document.getElementById('editor-scrollbar-track');
    const scrubTo = (clientX) => {
      const rect = track.getBoundingClientRect();
      const ratio = clamp((clientX - rect.left) / rect.width, 0, 1);
      this.editor.scrollX = ratio * this.editor._maxScrollX();
      this.editor._clampScroll();
    };
    track.addEventListener('touchstart', (e) => { e.preventDefault(); scrubTo(e.touches[0].clientX); }, { passive: false });
    track.addEventListener('touchmove', (e) => { e.preventDefault(); scrubTo(e.touches[0].clientX); }, { passive: false });
  ...
  _updateEditorZoomLabel() {
    document.getElementById('ed-zoom-label').textContent = Math.round(this.editor.zoom * 100) + '%';
  },

  _updateEditorScrollbar(scrollX, maxScrollX) {
    const track = document.getElementById('editor-scrollbar-track');
    const thumb = document.getElementById('editor-scrollbar-thumb');
    const trackWidth = track.clientWidth - 12; // minus padding
    const levelSpan = maxScrollX + this.editor.canvas.width;
    const thumbWidth = Math.max(30, (this.editor.canvas.width / levelSpan) * trackWidth);
    const maxThumbLeft = trackWidth - thumbWidth;
    const ratio = maxScrollX > 0 ? scrollX / maxScrollX : 0;
    thumb.style.width = thumbWidth + 'px';
    thumb.style.left = (6 + ratio * maxThumbLeft) + 'px';
  },

  _startEditorLoop() {
    const loop = () => {
      if (this.screens.editor.classList.contains('active')) {
        // Same safety net as the game screen: keep the backing canvas size
        // in sync with its container even if a resize event was missed.
        const wrap = document.getElementById('editor-canvas-wrap');
        const canvas = document.getElementById('editor-canvas');
        if (canvas.width !== wrap.clientWidth || canvas.height !== wrap.clientHeight) {
          this._resizeEditorCanvas();
  ...
  _bindEditor() {
    ...
    document.getElementById('ed-levelname').addEventListener('input', (e) => {
      this.editor.name = e.target.value || 'Untitled level';
    });
    document.getElementById('ed-clear').addEventListener('click', () => {
      if (confirm('Clear the editor?')) this.editor.clear();
    });
    ...
    document.getElementById('ed-save').addEventListener('click', () => {
      ...
      if (!data.name.trim()) { alert('Give your level a name!'); return; }
      const list = loadCustomLevels();
      list.push(data);
      saveCustomLevels(list);
      alert(`Level "${data.name}" saved! You'll find it under "My Levels".`);
    });
  },
});
